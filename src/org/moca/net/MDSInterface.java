...
public class MDSInterface {
	...
	public static boolean isProcedureAlreadyUploaded(Uri uri, Context context) {
		...
		for(Entry<String,Map<String,String>> e : elementMap.entrySet()) {
			...
			try {
				...
				ElementType eType = ElementType.valueOf(type);
				...
				case CAPTURE:
					...
					if (!"".equals(answer)) {
						...
						Log.d(TAG, "Element binary count: " + ids.length);
						totalBinaries += ids.length;
					}
					break;
				default:
					Log.d(TAG,"Not a binary type: " + eType.name());
					break;
				}
				
			} catch (Exception ex) {
				Log.e(TAG, "Couldn't parse type: " + ex.toString());
				ex.printStackTrace();
			}
		}
		...
		for(ElementAnswer e : binaries) {
			...
			for(String binaryId : ids) {
				...
				try {
					...
					Log.d(TAG,"Invalid element type: " + e.type);
				}
				...
				case SOUND:
					...
					break;
				case CAPTURE:
					binUri = ContentUris.withAppendedId(BinarySQLFormat.CONTENT_URI, Long.parseLong(binaryId));
					break;
				case BINARYFILE:
					binUri = ContentUris.withAppendedId(BinarySQLFormat.CONTENT_URI, Long.parseLong(binaryId));
				...
				try {
					...
					case SOUND:
						...
						break;
					case CAPTURE:
						cur = context.getContentResolver().query(binUri, new String[] { BinarySQLFormat.UPLOADED, BinarySQLFormat.UPLOAD_PROGRESS }, null, null, null);
						cur.moveToFirst();
						alreadyUploaded = cur.getInt(0) != 0;
						if (!alreadyUploaded) return false;
						break;
	...
	public static boolean postProcedureToDjangoServer(Uri uri, Context context) {
		...
		if(savedProcedureUploaded && false) {
	...
	private static int transmitBinary(Context c, String savedProcedureId, String elementId, String binaryGuid, ElementType type, Uri binaryUri, int startPacketSize) throws Exception {
		...
		case SOUND:
			...
			break;
		case CAPTURE:
			cur = c.getContentResolver().query(binaryUri, new String[] { BinarySQLFormat.UPLOADED, BinarySQLFormat.UPLOAD_PROGRESS }, null, null, null);
			cur.moveToFirst();
			alreadyUploaded = cur.getInt(0) != 0;
			currPosition = cur.getInt(1);
			cur.deactivate();
		...
		while(bytesRemaining > 0) {
			...
			case SOUND:
				...
				break;
			case CAPTURE:
				cv.put(BinarySQLFormat.UPLOAD_PROGRESS, currPosition);
				break;
			case BINARYFILE:
				cv.put(BinarySQLFormat.UPLOAD_PROGRESS, currPosition);
				break;
		...
		case SOUND:
			...
			break;
		case CAPTURE:
			cv.put(BinarySQLFormat.UPLOADED, true);
			break;
		case BINARYFILE:
			cv.put(BinarySQLFormat.UPLOADED, true);
			break;
