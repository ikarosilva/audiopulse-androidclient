...
public class TEOAEKempClientAnalysis {
	...
	static double Fs = 16000;	 					
	static double epochTime = 0.020;// Time units for search windows (in seconds) 			 
	static int epochSize = (int) Math.round(epochTime*Fs);
	...
	static int epochOnsetDelay= (int) Math.round(Fs*0.014);
	...
	public static double[] getTemporalAverage(double[] audioData){
		...
		System.out.println("Peak threshold= "+ peakThreshold);

		//Get peak indices
		List<Integer> peakInd = getPeakIndices(audioData,peakThreshold);
		System.out.println("Peaks found:" + peakInd.size());
	...
	public static double[] mainAnalysis(short[] rawData, double testFs, int testEpochSize) throws Exception 
	{
		...
		System.out.println("estimating evoked response");
		...
		System.out.println("TEOAE analysis complete!");
