...
public class DPOAEProcedure extends TestProcedure {
	...
	//calibrate one set of DPOAE tones
	private void calibrate(DPOAEParameters params) {
		double[][] f1Tone = AudioSignal.monoToStereoLeft(
				params.createF1Tone(playbackSampleFrequency, hardware) );
		double gainF1 = calibrate(f1Tone);
		double[][] f2Tone = AudioSignal.monoToStereoRight(
				params.createF1Tone(playbackSampleFrequency, hardware) );
		double gainF2 = calibrate(f2Tone);
	}
	
	//calibrate a single tone
	private double calibrate(double[][] signal) {
		testIO.setPlaybackAndRecording(playbackSampleFrequency, signal, 
				recordingSampleFrequency);
		//FIXME
		double aOut = hardware.getOutputLevel(signal[0]) + hardware.getOutputLevel(signal[1]);
		double[] input = testIO.acquire();
		double aIn = hardware.getInputLevel(input);
		return aIn - aOut;
	...
	public static final class BiologicParameters {
		...
		public static final DPOAEParameters F2k =
			new DPOAEParameters(protocol,2000,duration,6516,7969,64.8,54.9);
